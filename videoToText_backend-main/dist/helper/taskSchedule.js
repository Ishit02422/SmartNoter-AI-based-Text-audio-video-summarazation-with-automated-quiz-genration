...
const schema_2 = require("../modules/subscription/schema");
const cron = require("node-cron");
const moment = require("moment");
const taskSchedule = async (date, userId) => {
    // debugger;
    // console.log(userId, "**********");
    // console.log(date, "?????????????????");
    ...
    cron.schedule(`${minute} ${hour} ${day} ${month} *`, async () => {
        ...
        const inApp = await (0, subscription_1.getSubscriptionByUserId)(user._id);
        if (inApp.length > 0) {
            ...
            if (inApp[0].appType == "isIos") {
                ...
                if (status === 0) {
                    ...
                    if (validReceiptInfo) {
                        ...
                        if (!isAutoRenewable) {
                            console.log(user, "^^^^^^^^^^^^^^^^^^^^^^");
                            await (0, user_1.updateUser)(new schema_1.UserModel({
                                ...user,
                                isProUser: false,
                                isPurchased: false,
                                glitter: 0,
                ...
                if (regex.test(inApp[0].receiptId)) {
                    ...
                    if (!response.data.autoRenewing) {
                        ...
                        await (0, subscription_1.updateSubscription)(new schema_2.SubscriptionModel({
                            ...inApp[0].toObject(),
                        ...
                        if (inApp[0].subscriptionType == "WEEKLY") {
                            ...
                            await (0, exports.taskSchedule)(expiredTime, user._id);
                        }
                        else if (inApp[0].subscriptionType == "MONTHLY") {
                            expiredTime = moment()
                                .add(1, "months")
