"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const lodash_1 = require("lodash");
const image_1 = require("../../modules/image");
class Controller {
    constructor() {
        this.createImage = async (req, res) => {
            /*
              {
                fieldname: 'file',        String - name of the field used in the form
                originalname,             String - original filename of the uploaded image
                encoding,                 String - encoding of the image (e.g. "7bit")
                mimetype,                 String - MIME type of the file (e.g. "image/jpeg")
                buffer,                   Buffer - buffer containing binary data
                size,                     Number - size of buffer in bytes
                filename,                 String - file name
                filepath                  String - file path
              }
            */
            try {
                const authUser = req.authUser;
                if (!authUser) {
                    return res.status(403).json("unauthorized request");
                }
                const file = req.files[0];
                const image = await (0, image_1.createAndUploadImage)(file, req.body.title, req.body.description, authUser._id);
                const resImage = await (0, image_1.getImageById)(image._id);
                console.log("resImage", resImage);
                return res.status(200).json(resImage);
            }
            catch (err) {
                console.log("########## Error in createImage", err);
                return res.status(500).json({ error: (0, lodash_1.get)(err, "message") });
            }
        };
        this.createImageForImageToPrompt = async (req, res) => {
            /*
              {
                fieldname: 'file',        String - name of the field used in the form
                originalname,             String - original filename of the uploaded image
                encoding,                 String - encoding of the image (e.g. "7bit")
                mimetype,                 String - MIME type of the file (e.g. "image/jpeg")
                buffer,                   Buffer - buffer containing binary data
                size,                     Number - size of buffer in bytes
                filename,                 String - file name
                filepath                  String - file path
              }
            */
            try {
                const authUser = req.authUser;
                if (!authUser) {
                    return res.status(403).json("unauthorized request");
                }
                const file = req.files[0];
                const image = await (0, image_1.createAndUploadImageAndThumbnail)(file, "IMAGE-TO-PROMPT", "IMAGE-TO-PROMPT", authUser._id);
                const resImage = await (0, image_1.getImageById)(image._id);
                return res.status(200).json(resImage);
            }
            catch (err) {
                console.log("########## Error in createImageForImageToPrompt", err);
                return res.status(500).json({ error: (0, lodash_1.get)(err, "message") });
            }
        };
        // protected readonly deleteImage = async (req: Request, res: Response) => {
        //   try {
        //     //
        //   } catch (err) {
        //     console.log("########## Error in deletingImage", err);
        //     res.status(500).json({ error: _get(err, "message") });
        //   }
        //   // var s3 = AWS.S3(awsCredentials);
        //   // s3.deleteObject(
        //   //   {
        //   //     Bucket: MY_BUCKET,
        //   //     Key: "some/subfolders/nameofthefile1.extension",
        //   //   },
        //   //   function (err, data) {}
        //   // );
        // };
    }
}
exports.default = Controller;
//# sourceMappingURL=image.controller.js.map